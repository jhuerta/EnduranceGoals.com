...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <fieldset>
        <legend><%= Html.Encode(Model.Name) %> <b>(<%= Html.ActionLink("edit", "edit", new { id = Model.Id })%> )  </b> </legend>
        <ul>
            ...
            <li><span class="display-label">When: </span><span class="display-field">
                ...
                <%= Html.Encode(String.Format("{0}, {1} ({2})", Model.Venue, Model.Venue.City, Model.Venue.City.Country))%></span> </li>
            <li><span class="display-label">Added By: </span><span class="display-field">
                <%= Html.Encode(String.Format("{0:D}", Model.UserCreator.Username)) %></span> </li>
                            <li><span class="display-label">Current Participants: </span><span class="display-field">
                <%= Html.Encode(String.Format("{0}", string.Join(",", Model.Participants.Select(participant => participant.User.Username).ToArray())))%></span> </li>
